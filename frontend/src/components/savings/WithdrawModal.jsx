...
const WithdrawModal = ({ isOpen, onClose, saving, onSuccess }) => {
  ...
  const { withdrawFromSaving } = useSavings();
  ...
  const validateForm = () => {
    ...
    } else if (parseFloat(formData.amount) > (saving.currentAmount || 0)) {
      newErrors.amount = `Số tiền không được vượt quá ${savingsService.formatAmount(saving.currentAmount || 0, saving.currency)}`;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await withdrawFromSaving(saving.id, {
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        <div className={styles.header}>
          <h2>Rút tiền từ {saving.title}</h2>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            <label className={styles.label}>
              Số tiền rút <span className={styles.required}>*</span>
            </label>
          ...
          <div className={styles.formGroup}>
            <label className={styles.label}>
              Ví nhận tiền <span className={styles.required}>*</span>
            </label>
            <select
              name="walletId"
